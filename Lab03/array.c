...
int* bubbleSort(int arr[], int n) {
    ...
    //Use array format *(arr + n)
    for (i = 0; i < n - 1; i++) {
        for (j = 0; j < n - 1; j++) {
            if(*(s_arr + j) > *(s_arr + j + 1)) { //Compare 1st & 2nd //Ex. [3, 2], 3>2 so [2,3]
                temp = *(s_arr + j + 1);  //Hold 2nd Value
                *(s_arr + j + 1) = *(s_arr + j); //Swap 2nd Value to 1st Value 
                *(s_arr + j) = temp; //Set First Value to 2nds
            }
        }
    }
...
int bSearch(int *arr, int a, int b, int key) {
    ...
    // Your code goes here:
    if (*(arr + a) == key) { //Check if its in first index
        return a;
    }

    else if(*(arr + a) != key && a < b) { //Check next index
        if (*(arr + a) < key){
            a++;
