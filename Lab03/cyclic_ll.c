...
int has_cycle(node *head) {
	// Your code goes here:
  if (head == NULL) { //If head is null no cycle
    return 0;
  }
    ...
    while(temp != NULL && nextNode != NULL){ //As long as temp and nextNode not null search for a cycle
      if(temp == nextNode){ //If second pointer is same as first, cycle
        return 1;
      } 
      else if(nextNode->next == temp){ //If next node of secondpointer is pointed by first pointer, cycle
			return 1;
		  }
      else if(nextNode->next == NULL){ //if second pointer next is null, no cycle
			return 0;
		}
